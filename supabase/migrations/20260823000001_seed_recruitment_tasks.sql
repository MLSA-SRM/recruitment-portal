-- Migration: Seed recruitment tasks for Technical, Corporate, and Creative domains
-- Description: One row per (task, year) since target_year has no 'all years' concept yet
-- Created: 2026-08-23
...
VALUES ('Live-Sync Mini App', 'Build a small app where multiple open browser tabs see each other''s actions in real time: a shared sticky-note board, live cursor positions, or a shared poll that updates as votes come in. Use React for the interface and either WebSockets or Server-Sent Events for the real-time part. Two tabs open side by side should be enough to demo it.', 'Technical', 'Web Development: Frontend', 1, '2026-09-09T23:59:59.999Z'::timestamptz, 'Node 18 or newer. A WebSocket server is easiest with a small Express or Fastify backend and the `ws` library, or Socket.IO if you''d rather have reconnection handling built in. If you go the SSE route, you only need a single HTTP endpoint that stays open and streams events.

Resources: WebSocket API (MDN) https://developer.mozilla.org/en-US/docs/Web/API/WebSockets_API | Server-Sent Events (MDN) https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events | Socket.IO docs https://socket.io/docs/v4/', 'GitHub repo, README, and a 2-3 min video walkthrough (see submission fields). Include a one-line note on how you resolved simultaneous edits, if applicable.', '4-6 hours')
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Live-Sync Mini App', 'Build a small app where multiple open browser tabs see each other''s actions in real time: a shared sticky-note board, live cursor positions, or a shared poll that updates as votes come in. Use React for the interface and either WebSockets or Server-Sent Events for the real-time part. Two tabs open side by side should be enough to demo it.

Second-year build-on: Add optimistic UI updates, and handle two tabs editing the same note or cursor position at the same moment without one silently overwriting the other. Then make the state survive a refresh: closing a tab and reopening it should restore where things were, not reset to empty.', 'Technical', 'Web Development: Frontend', 2, '2026-09-09T23:59:59.999Z'::timestamptz, 'Node 18 or newer. A WebSocket server is easiest with a small Express or Fastify backend and the `ws` library, or Socket.IO if you''d rather have reconnection handling built in. If you go the SSE route, you only need a single HTTP endpoint that stays open and streams events.
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Fix This Broken UI', 'We''ll hand you a small React app with four bugs planted in it on purpose: a stale closure that shows the wrong value, a missing list key causing weird re-render behavior, a layout bug that only appears on narrow screens, and an accessibility issue a screen reader would catch. Find all four, fix them, and add a one-line comment at each fix explaining what was actually wrong. This is a debugging task, not a rebuild: please don''t rewrite the app from scratch, we specifically want to see how you diagnose someone else''s code.', 'Technical', 'Web Development: Frontend', 1, '2026-09-09T23:59:59.999Z'::timestamptz, 'Node 18 or newer, and a browser with dev tools (React DevTools is genuinely useful here). For the accessibility bug, VoiceOver on macOS or NVDA on Windows will catch it directly, but the axe DevTools browser extension works too.

Resources: React docs on state and closures https://react.dev/learn | ARIA Authoring Practices Guide https://www.w3.org/WAI/ARIA/apg/ | axe DevTools https://www.deque.com/axe/devtools/', 'GitHub repo with the four fixes and one-line comments explaining each, README, and a video walkthrough pointing at each bug''s actual line.', '2-4 hours')
RETURNING id INTO new_task_id;
INSERT INTO public.submission_fields (task_id, field_name, field_type, field_label, field_description, is_required, display_order)
VALUES (new_task_id, 'github_link', 'url', 'GitHub Repository Link', 'Public repo, or add the recruitment GitHub account as a collaborator if private.', true, 1);
INSERT INTO public.submission_fields (task_id, field_name, field_type, field_label, field_description, is_required, display_order)
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Fix This Broken UI', 'We''ll hand you a small React app with four bugs planted in it on purpose: a stale closure that shows the wrong value, a missing list key causing weird re-render behavior, a layout bug that only appears on narrow screens, and an accessibility issue a screen reader would catch. Find all four, fix them, and add a one-line comment at each fix explaining what was actually wrong. This is a debugging task, not a rebuild: please don''t rewrite the app from scratch, we specifically want to see how you diagnose someone else''s code.

Second-year build-on: After the four fixes, add one small new feature to the same app (your choice, keep it small). The new feature has to be keyboard-accessible on its own, and covered by at least one automated test using React Testing Library.', 'Technical', 'Web Development: Frontend', 2, '2026-09-09T23:59:59.999Z'::timestamptz, 'Node 18 or newer, and a browser with dev tools (React DevTools is genuinely useful here). For the accessibility bug, VoiceOver on macOS or NVDA on Windows will catch it directly, but the axe DevTools browser extension works too.
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Accessible Custom Component', 'Build one non-trivial UI component completely from scratch, no component library: a multi-select dropdown, a virtualized long list, or a drag-to-reorder list. It needs full keyboard navigation and correct focus management, not just mouse support.', 'Technical', 'Web Development: Frontend', 1, '2026-09-09T23:59:59.999Z'::timestamptz, 'Just React and Node 18 or newer. Deliberately skip Radix, MUI, and similar libraries for this one, the point is building the primitive yourself.

Resources: ARIA Authoring Practices Guide https://www.w3.org/WAI/ARIA/apg/ | MDN keyboard-accessible guide https://developer.mozilla.org/en-US/docs/Web/Accessibility/Guides/Keyboard-navigable_JavaScript', 'GitHub repo, README, and a video walkthrough. A short Storybook page or plain demo page is welcome but not required.', '3-5 hours')
RETURNING id INTO new_task_id;
INSERT INTO public.submission_fields (task_id, field_name, field_type, field_label, field_description, is_required, display_order)
VALUES (new_task_id, 'github_link', 'url', 'GitHub Repository Link', 'Public repo, or add the recruitment GitHub account as a collaborator if private.', true, 1);
INSERT INTO public.submission_fields (task_id, field_name, field_type, field_label, field_description, is_required, display_order)
...
Second-year build-on: Make the component work correctly with a screen reader, not just a keyboard. Then support both a controlled and an uncontrolled usage mode, the way real component libraries do (compare how a native <input> supports both value and defaultValue).', 'Technical', 'Web Development: Frontend', 2, '2026-09-09T23:59:59.999Z'::timestamptz, 'Just React and Node 18 or newer. Deliberately skip Radix, MUI, and similar libraries for this one, the point is building the primitive yourself.
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Optimize This Slow Endpoint', 'We''ll hand you a small, working API with one endpoint that''s deliberately slow: either an N+1 query problem or a missing index. Profile it, find the actual cause, fix it, and write down the before and after numbers.', 'Technical', 'Web Development: Backend', 1, '2026-09-09T23:59:59.999Z'::timestamptz, 'Node 18+ or Python 3.10+ depending on which starter you''re given, plus whatever database the starter uses (local SQLite or Postgres, nothing to provision). `console.time`/`console.timeEnd` or Python''s `time.perf_counter` is genuinely enough.

Resources: Postgres EXPLAIN https://www.postgresql.org/docs/current/using-explain.html | The N+1 query problem, explained https://planetscale.com/blog/what-is-n-1-query-problem-and-how-to-solve-it', 'GitHub repo, README with your before/after numbers, and a video walkthrough.', '2-4 hours')
RETURNING id INTO new_task_id;
INSERT INTO public.submission_fields (task_id, field_name, field_type, field_label, field_description, is_required, display_order)
VALUES (new_task_id, 'github_link', 'url', 'GitHub Repository Link', 'Public repo, or add the recruitment GitHub account as a collaborator if private.', true, 1);
INSERT INTO public.submission_fields (task_id, field_name, field_type, field_label, field_description, is_required, display_order)
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Optimize This Slow Endpoint', 'We''ll hand you a small, working API with one endpoint that''s deliberately slow: either an N+1 query problem or a missing index. Profile it, find the actual cause, fix it, and write down the before and after numbers.

Second-year build-on: Add caching in front of the fixed endpoint (in-memory with a TTL is fine, Redis if you want the practice) and write a short load test showing throughput with and without the cache. Resources: Redis docs https://redis.io/docs/latest/ | k6 docs https://k6.io/docs/', 'Technical', 'Web Development: Backend', 2, '2026-09-09T23:59:59.999Z'::timestamptz, 'Node 18+ or Python 3.10+ depending on which starter you''re given, plus whatever database the starter uses (local SQLite or Postgres, nothing to provision). `console.time`/`console.timeEnd` or Python''s `time.perf_counter` is genuinely enough.
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('API in an Unfamiliar Framework', 'Build a small CRUD API with authentication, but in a framework you haven''t used before. If you''re comfortable in Node/Express, try FastAPI. If you''re a Python person, try Express or a minimal Go HTTP server. The concepts (routing, validation, auth, status codes) are ones you already know, the point is proving you can carry them into new syntax.', 'Technical', 'Web Development: Backend', 1, '2026-09-09T23:59:59.999Z'::timestamptz, 'Whatever runtime the framework needs (Node 18+, Python 3.10+, or Go 1.21+). Keep auth simple, a signed JWT or a hardcoded API key check is fine.

Resources: FastAPI docs https://fastapi.tiangolo.com/ | Express docs https://expressjs.com/ | Go net/http https://pkg.go.dev/net/http', 'GitHub repo, README noting which framework you picked and one thing that surprised you, and a video walkthrough.', '3-5 hours')
RETURNING id INTO new_task_id;
INSERT INTO public.submission_fields (task_id, field_name, field_type, field_label, field_description, is_required, display_order)
VALUES (new_task_id, 'github_link', 'url', 'GitHub Repository Link', 'Public repo, or add the recruitment GitHub account as a collaborator if private.', true, 1);
INSERT INTO public.submission_fields (task_id, field_name, field_type, field_label, field_description, is_required, display_order)
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('API in an Unfamiliar Framework', 'Build a small CRUD API with authentication, but in a framework you haven''t used before. If you''re comfortable in Node/Express, try FastAPI. If you''re a Python person, try Express or a minimal Go HTTP server. The concepts (routing, validation, auth, status codes) are ones you already know, the point is proving you can carry them into new syntax.

Second-year build-on: Add rate limiting and structured logging (JSON logs with a request ID on every line), then containerize the whole thing with a working Dockerfile someone else could docker run without reading your code first. Resources: Docker getting started https://docs.docker.com/get-started/', 'Technical', 'Web Development: Backend', 2, '2026-09-09T23:59:59.999Z'::timestamptz, 'Whatever runtime the framework needs (Node 18+, Python 3.10+, or Go 1.21+). Keep auth simple, a signed JWT or a hardcoded API key check is fine.
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Background Job Queue', 'Build a small job queue: something submits jobs (an API endpoint is fine), a worker processes them asynchronously, and failed jobs retry a couple of times before giving up. BullMQ with Redis is the well-trodden path, but a simpler cron-plus-database-polling approach is completely acceptable.', 'Technical', 'Web Development: Backend', 1, '2026-09-09T23:59:59.999Z'::timestamptz, 'Node 18+ (or Python with Celery), and Redis if you go the BullMQ route (a free Upstash Redis instance works, no local install needed).

Resources: BullMQ docs https://docs.bullmq.io/ | Upstash Redis (free tier) https://upstash.com/ | Celery docs https://docs.celeryq.dev/en/stable/', 'GitHub repo, README, and a video walkthrough that actually triggers a job that fails on purpose.', '3-5 hours')
RETURNING id INTO new_task_id;
INSERT INTO public.submission_fields (task_id, field_name, field_type, field_label, field_description, is_required, display_order)
VALUES (new_task_id, 'github_link', 'url', 'GitHub Repository Link', 'Public repo, or add the recruitment GitHub account as a collaborator if private.', true, 1);
INSERT INTO public.submission_fields (task_id, field_name, field_type, field_label, field_description, is_required, display_order)
...
Second-year build-on: Add a dead-letter queue for jobs that fail every retry, and build a tiny status page (even a single unstyled HTML page is fine) showing pending, running, succeeded, and dead job counts.', 'Technical', 'Web Development: Backend', 2, '2026-09-09T23:59:59.999Z'::timestamptz, 'Node 18+ (or Python with Celery), and Redis if you go the BullMQ route (a free Upstash Redis instance works, no local install needed).
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('RAG Mini Q&A Bot', 'We''ll give you a small set of documents. Build a question-answering tool that actually retrieves relevant passages from those specific documents before answering, rather than just forwarding the question straight to an LLM. We want to see the retrieval step, not just a wrapped API call.

Second-year build-on: Show which document and passage each answer actually came from (a citation, not just a confident-sounding answer). Then handle the case where the answer genuinely isn''t in the documents: the bot should say so rather than making something up.', 'Technical', 'AI/ML', 2, '2026-09-09T23:59:59.999Z'::timestamptz, 'Python 3.10+, an embeddings model (OpenAI''s embeddings API, or a free local model via sentence-transformers), and a simple vector store (even an in-memory list with cosine similarity is fine at this scale).
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Messy Dataset, Baseline Model', 'We''ll give you a small, deliberately messy CSV (missing values, a few outliers, inconsistent types in places). Explore it, clean what needs cleaning, build a baseline classifier or regressor, and write up what you found and why you made the choices you made. We''re evaluating your reasoning here as much as your model''s accuracy.

Second-year build-on: Compare at least two different modeling approaches and justify, specifically for this dataset, which one you''d actually ship. Add a short error analysis: what does the model get wrong, and what''s the pattern.', 'Technical', 'AI/ML', 2, '2026-09-09T23:59:59.999Z'::timestamptz, 'Python 3.10+, pandas, and scikit-learn. A Jupyter notebook or Google Colab is the natural format.
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Sponsorship Trade-offs', 'Shared scenario: MSA SRM is planning its flagship two-day hackathon for October 2026, expecting 400+ participants across roughly 80 teams. Recruitment is happening 9 weeks out, and the core team is short-staffed for an event of this scale. You''ve just joined the team five days ago.', 'Corporate', 'Sponsorship', 2, '2026-09-09T23:59:59.999Z'::timestamptz, 'Task 1: Three companies have responded to outreach, and you have 12 days until sponsor branding must be finalized for print.
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Event-Day Contingency Planning', 'Shared scenario: MSA SRM is planning its flagship two-day hackathon for October 2026, expecting 400+ participants across roughly 80 teams. Recruitment is happening 9 weeks out, and the core team is short-staffed for an event of this scale. You''ve just joined the team five days ago.', 'Corporate', 'Event Management & Logistics', 2, '2026-09-09T23:59:59.999Z'::timestamptz, 'Task 1: Nine days before the event, three things land at once.
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Resolving Cross-Team Conflicts', 'Shared scenario: MSA SRM is planning its flagship two-day hackathon for October 2026, expecting 400+ participants across roughly 80 teams. Recruitment is happening 9 weeks out, and the core team is short-staffed for an event of this scale. You''ve just joined the team five days ago.', 'Corporate', 'Operations', 2, '2026-09-09T23:59:59.999Z'::timestamptz, 'Task 1: All six club verticals submit event requirements into one shared spreadsheet. For this event, three verticals have each submitted conflicting requirements for the same volunteer pool on the same day, each assuming they''d get first pick.
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Handling a PR Incident', 'Shared scenario: MSA SRM is planning its flagship two-day hackathon for October 2026, expecting 400+ participants across roughly 80 teams. Recruitment is happening 9 weeks out, and the core team is short-staffed for an event of this scale. You''ve just joined the team five days ago.', 'Corporate', 'PR & Outreach', 2, '2026-09-09T23:59:59.999Z'::timestamptz, 'Task 1: Two hours ago, a meme posted on the club''s Instagram poking fun at a rival college''s tech fest picked up angry comments, including one from a faculty member calling it "unprofessional and embarrassing for the institution." The post already has 3,000+ organic likes.
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Rewriting an Event Announcement', 'Shared scenario: MSA SRM is planning its flagship two-day hackathon for October 2026, expecting 400+ participants across roughly 80 teams. Recruitment is happening 9 weeks out, and the core team is short-staffed for an event of this scale. You''ve just joined the team five days ago.', 'Corporate', 'Content Writing', 2, '2026-09-09T23:59:59.999Z'::timestamptz, 'Task 1: Last year''s hackathon announcement opened with: "Get ready for an epic 24 hours of code, chaos, and cracking the case!" For reference only, that post picked up 47 shares.
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Reimagine an Event', 'Pick any past or upcoming SRM MSA event and redesign its poster / social media creative from scratch (an original take, not a touch-up).', 'Creatives', 'Graphic Design', 1, '2026-09-09T23:59:59.999Z'::timestamptz, 'Use Microsoft brand colors without directly copying official MS templates.
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Reimagine an Event', 'Pick any past or upcoming SRM MSA event and redesign its poster / social media creative from scratch (an original take, not a touch-up).', 'Creatives', 'Graphic Design', 2, '2026-09-09T23:59:59.999Z'::timestamptz, 'Use Microsoft brand colors without directly copying official MS templates.
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Tell a Story in 5 Frames', 'Shoot a 5-photo series around one theme: "Campus Life," "Behind the Scenes of Tech," or "A Day in the Life of a Student Ambassador."', 'Creatives', 'Photography', 2, '2026-09-09T23:59:59.999Z'::timestamptz, 'Show range: at least one candid shot, one composed/staged shot, and one detail/macro shot.
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('One Event, Two Moods', 'Take one event brief (e.g., "Hackathon Kickoff") and design two poster versions with distinct tones: one energetic/bold and one minimal/professional.', 'Creatives', 'UI/UX Design', 1, '2026-09-09T23:59:59.999Z'::timestamptz, 'Both posters must be clearly derived from the same event brief but visually distinct in tone.
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('One Event, Two Moods', 'Take one event brief (e.g., "Hackathon Kickoff") and design two poster versions with distinct tones: one energetic/bold and one minimal/professional.', 'Creatives', 'UI/UX Design', 2, '2026-09-09T23:59:59.999Z'::timestamptz, 'Both posters must be clearly derived from the same event brief but visually distinct in tone.
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Add the Hook', 'Take a video clip without a strong opening and edit the first 3 seconds to immediately grab attention (using cuts, text, zooms, audio hooks, etc.).', 'Creatives', 'Videography', 1, '2026-09-09T23:59:59.999Z'::timestamptz, 'Source clip can be your own footage or any royalty-free clip, credit the source if it isn''t yours.
...
BEGIN
INSERT INTO public.tasks (title, description, domain, subdomain, target_year, deadline, requirements, deliverables, estimated_duration)
VALUES ('Add the Hook', 'Take a video clip without a strong opening and edit the first 3 seconds to immediately grab attention (using cuts, text, zooms, audio hooks, etc.).', 'Creatives', 'Videography', 2, '2026-09-09T23:59:59.999Z'::timestamptz, 'Source clip can be your own footage or any royalty-free clip, credit the source if it isn''t yours.
